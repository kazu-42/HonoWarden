...
const checkedAt = '2026-07-20 07:55 JST'
...
const body = [
  ...
  '- An active account with a non-empty wrapped user key and both-null account-key state may initialize once. Missing wrapped user key fails before mutation. An exact replay is a no-op. Partial state or any different value is a non-disclosing conflict and cannot overwrite.',
  ...
  '- Disabled account-key GET/HEAD/POST bypass the optional global quota and remain D1-free 501. Typed projection corruption emits a redacted request-correlated signal, and backup failure audit preserves its bounded reason before generic 503.',
  '',
  'Completion gates:',
