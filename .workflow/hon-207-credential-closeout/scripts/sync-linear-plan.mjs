...
if (mode === '--render') {
  const identifiers = Object.fromEntries(
    hon207LinearPlan.issues.map((issue) => [issue.key, issue.key]),
  )
  console.log(
    JSON.stringify(
      {
        ...
        descriptions: hon207LinearPlan.issues.map((issue) => ({
          key: issue.key,
...
async function applyPlan(before) {
  ...
  for (const definition of hon207LinearPlan.issues) {
    ...
    const data = await request(
      `mutation CreateCloseoutPacket($input: IssueCreateInput!) {
...
async function readParent() {
  const data = await request(
    `query ReadCloseoutParent($id: String!) {
      issue(id: $id) {
        id identifier title description priority archivedAt url
        parent { id identifier }
        project { id name }
    ...
    { id: hon207LinearPlan.parentIdentifier },
  )
  if (!data.issue) {
    throw new Error('HON-207 was not found')
  }
...
async function readInventory(teamId) {
  ...
  do {
    const data = await request(
      `query ReadCloseoutInventory($teamId: ID!, $after: String) {
        issues(
...
function assertParent(parent) {
  const checks = {
    id: parent.id === hon207LinearPlan.parentId,
    identifier: parent.identifier === hon207LinearPlan.parentIdentifier,
    title: parent.title === hon207LinearPlan.parentTitle,
    project: parent.project?.name === hon207LinearPlan.projectName,
    state: parent.state.type === 'started',
    priority: parent.priority === hon207LinearPlan.priority,
  ...
  if (failures.length > 0) {
    throw new Error(`refusing HON-207 mutation: ${failures.join(', ')}`)
  }
}
...
function assertExistingChildrenMutable(issueByKey, parent, workflowStates) {
  for (const definition of hon207LinearPlan.issues) {
    ...
    if (issue.priority !== hon207LinearPlan.priority) failures.push('priority')
    if (!expected?.id) {
...
async function ensureRelations(issueByKey) {
  ...
  for (const pair of expectedRelationPairs(issueByKey)) {
    ...
    const data = await request(
      `mutation CreateCloseoutRelation($input: IssueRelationCreateInput!) {
...
async function readRelationConnection(issueId, connectionName) {
  ...
  while (hasNextPage) {
    const data = await request(
      `query ReadCloseoutRelations($id: String!, $after: String) {
        issue(id: $id) {
...
async function syncManagedComment(parentId, body) {
  ...
  if (managed.length > 1) {
    throw new Error(`duplicate managed HON-207 comments: ${managed.length}`)
  }
  if (managed.length === 1) {
    ...
    const data = await request(
      `mutation UpdateCloseoutCheckpoint($id: String!, $input: CommentUpdateInput!) {
    ...
    if (
      ...
      throw new Error('Linear did not update the managed HON-207 comment')
    }
    return
  }
  const data = await request(
    `mutation CreateCloseoutCheckpoint($input: CommentCreateInput!) {
  ...
  if (
    ...
    throw new Error('Linear did not create the managed HON-207 comment')
  }
}
...
async function readAllComments(issueId) {
  ...
  while (hasNextPage) {
    const data = await request(
      `query ReadCloseoutComments($id: String!, $after: String) {
        issue(id: $id) {
...
async function updateIssue(id, input) {
  const data = await request(
    `mutation UpdateCloseoutIssue($id: String!, $input: IssueUpdateInput!) {
...
async function request(query, variables) {
  ...
  if (!response.ok || payload.errors) {
    ...
    throw new Error(`HON-207 Linear plan sync failed: ${messages.join('; ')}`)
  }
  return payload.data
}
...
async function writeReadback(verification, phase) {
  const readback = {
    ...
    sourcePins: hon207LinearPlan.sourcePins,
