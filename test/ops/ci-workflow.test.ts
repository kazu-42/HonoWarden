...
describe('ci workflow', () => {
  it('contains external brand scan protection and does not run destructive git steps', () => {
    ...
    expect(workflow).toContain("'[Bb]it'")
    expect(workflow).toContain("'warden|BIT'")
    expect(workflow).toContain("'WARDEN|Bit'")
    expect(workflow).toContain("'warden|bit'")
    expect(workflow).toContain("'warden')")
    expect(workflow).toContain('Format check')
    expect(workflow).toContain('pnpm format')
    expect(workflow).toContain('--glob "!node_modules/**"')
    expect(workflow).toContain('--glob "!pnpm-lock.yaml"')
    expect(workflow).toContain('--glob "!LICENSE"')
    expect(workflow).toContain('--glob "!dist/**"')
    expect(workflow).toContain('--glob "!coverage/**"')
    expect(workflow).toContain('--glob "!test/.tmp/**"')
