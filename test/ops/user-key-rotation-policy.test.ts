...
describe('user-key rotation operator policy', () => {
  ...
  it('documents activation, status, incident, and forward-recovery boundaries', () => {
    ...
    expect(dataFlow).toContain('six snapshot queries')
    expect(dataFlow).toMatch(/eleven transactional statements/i)
    expect(dataFlow).toMatch(/R2\s+object keys and bytes remain unchanged/i)
