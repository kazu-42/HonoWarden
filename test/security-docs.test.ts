...
describe('security review materials', () => {
  ...
  it('pins temporary patched transitive dependencies', () => {
    ...
    expect(dependencyAudit).toContain('GHSA-2v37-7h3g-55p8')
  ...
  it('records critical security review sections', () => {
    ...
    expect(threatModel).toContain('ADR 0010')
    ...
    expect(knownLimitations).toMatch(/cross-user\s+isolation/)
    expect(knownLimitations).toContain('policy schema')
    expect(knownLimitations).toMatch(/organization collection\s+CRUD/i)
    expect(knownLimitations).toContain('organization cipher assignment')
    ...
    expect(knownLimitations).toContain('atomically supersedes')
    expect(knownLimitations).toContain('D1 `audit_events`')
    expect(knownLimitations).toContain('HonoWarden-inquiry-inbox')
