...
- `/api/organizations` and `/api/organizations/*` return typed
  unsupported-feature errors. Organizations and shared vaults must not be
...
7. Shared vault privilege escalation.
   Current mitigation: Organization routes remain unsupported. ADR 0005
   requires membership, ownership, role, collection-access, cross-user
