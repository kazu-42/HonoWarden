...
- hosted billing, commercial licensing, provider/reseller portals,
  sponsorships, and multi-tenant hosted operation; ADR 0014 keeps only the
  truthful zero-cost startup subscription read
...
- Hosted billing mutations, licenses, plans, provider/reseller portals,
  sponsorships, and `/api/billing/*` return typed `501` unsupported-feature
  errors under ADR 0014. The only billing-shaped success path is authenticated
...
| Payment or tenancy confusion | ADR 0014 keeps startup billing unpaid and canceled; hosted billing, licensing, provider/reseller, and multi-tenant routes return explicit `501` before payment or tenant work                                                                             | a future hosted product would need tenant isolation and payment compliance before any runtime                                                     |
