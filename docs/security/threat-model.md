...
| Repudiation            | opt-in D1-persisted audit events for bootstrap, auth failures, refresh reuse, backup export, folder/cipher/attachment mutations, device revoke, revoke-all-other-sessions, TOTP change, and TOTP disable | audit coverage does not yet include unsupported organization/public-sharing surfaces or external log sink |
