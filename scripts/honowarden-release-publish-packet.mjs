...
function main(argv = process.argv.slice(2)) {
  ...
  const report = buildPublishPacket(options)

  process.stdout.write(`${JSON.stringify(report, null, 2)}\n`)

  if (options.strict && report.status !== 'ready') {
    process.stderr.write(
      `release publish packet is not ready: ${failedCount(report)} failed check(s)\n`,
    )
    process.exitCode = 1
  }
}
...
function buildPublishPacket(options) {
  ...
  return {
    ...
    publishApprovalText:
      ready && release.release
        ? `${targetCommit} の ${targetTag} draft prerelease を公開してよい`
        : null,
    limitations: [
      'This packet does not publish, update, or delete a GitHub release.',
      'This packet does not create, move, delete, or push a Git tag.',
      'Run the publish command only after explicit operator approval.',
      'Deploying from a release requires a separate approval gate.',
    ],
  }
}
...
function resolveReleaseState(targetCommit) {
  ...
  if (normalizedRelease.isDraft !== true) {
    stateFailures.push(`isDraft=${normalizedRelease.isDraft}; expected true`)
  }
  ...
  return {
    ...
    stateCheck: check(
      ...
      stateFailures.length === 0
        ? 'existing release is a draft prerelease for the target commit'
        : stateFailures.join('; '),
    ),
    bodyCheck: check(
      'release_body',
...
function printUsage() {
  ...
  node scripts/honowarden-release-publish-packet.mjs [--strict] [--remote <remote>] [--expected-commit <sha>] [--tag-workflow-run-id <id>] [--tag-workflow-url <url>] [--no-default-tag-workflow-evidence]
`)
}
